...
func TestEncryptAtPathReadsBackWhatDecryptWrote(t *testing.T) {
	...
	for _, want := range values {
		t.Run(strconv.Quote(want), func(t *testing.T) {
			...
			encrypted, err := EncryptAtPath(doc, path, testPassword, false)
			if err != nil {
...
// decryptedDocHolding returns pathTestDoc with value written at path by DecryptAtPath, which is the
...
func decryptedDocHolding(t *testing.T, path, value string) []byte {
	...
	ciphertext, err := EncryptValue(value, testPassword)
	if err != nil {
	...
	decrypted, err := DecryptAtPath(doc, path, testPassword)
	if err != nil {
		t.Fatalf("DecryptAtPath() error = %v", err)
	}
