...
func runEngineConfigSync(ctx context.Context, exec Exec, args *Args, resp *Response) error {
	...
	return runAction(ctx, exec, resp, &p.common, args.Control,
		func(inventory string) []string {
			return buildEngineConfigSyncArgs(&p, inventory, args.Control)
		},
		messages{
			done:  "synchronised engine configuration across the fleet",
...
func buildEngineConfigSyncArgs(p *engineConfigSyncParams, inventory string, control Control) []string {
	c := begin("engine-config-sync", []string{p.Package}, inventory, control,
