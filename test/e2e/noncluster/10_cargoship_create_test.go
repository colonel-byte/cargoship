...
func TestCargoshipCreate(t *testing.T) {
	...
	t.Run("creates a package with overridden tag", func(t *testing.T) {
		outDir := t.TempDir()
		_, _, err := e2e.Cargoship(t, "create", minimalDistroDir, "-o", outDir, "--tag", "0.0.1-custom-tag")
		require.NoError(t, err)

		matches, err := filepath.Glob(filepath.Join(outDir, "*.tar.zst"))
		require.NoError(t, err)
		require.Len(t, matches, 1)
		require.Equal(t, "cargoship-e2e-minimal-"+e2e.Arch+"-0.0.1-custom-tag.tar.zst", filepath.Base(matches[0]))
		...
		distroYAML, err := os.ReadFile(filepath.Join(extractDir, "distro.yaml"))
		require.NoError(t, err)
		require.Contains(t, string(distroYAML), "version: 0.0.1-custom-tag")
	})
...
// release tarballs and every pinned image for both architectures the example covers --
// several gigabytes. Skipped under -short so PR CI can run the rest of the suite in
// seconds.
func TestCargoshipCreateExample(t *testing.T) {
	if testing.Short() {
		t.Skip("example packages download ~1.5GB of engine artifacts and images")
	}
	t.Setenv("CARGOSHIP_CONFIG", "test/e2e/cargoship-config.yaml")

	_, _, err := e2e.Cargoship(t, "create", "example/rke2-multi-cni-cilium/v1_36/v1.36.4-rke2r1")
	require.NoError(t, err)
}
