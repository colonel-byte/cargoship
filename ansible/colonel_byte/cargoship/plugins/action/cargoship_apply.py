...
"""Action plugin for the cargoship_apply module.
...
short_description: Install or converge a Kubernetes cluster across the fleet
description:
  - "Runs C(cargoship apply) against the fleet described by an Ansible inventory. It is the widest action cargoship has."
...
options:
  ...
  allow_unmanaged_nodes:
    description:
      - "Whether to tolerate nodes in the cluster that the inventory does not describe."
    type: bool
    required: false
    cli_flag: "--allow-unmanaged-nodes"
  update_kubeconfig:
    description:
...
- name: Install the cluster
  colonel_byte.cargoship.cargoship_apply:
