...
`kubernetes.core` is installed and working, along with `helm` and `kubectl`: the base carries the full ansible distribution, and the image adds the Python client the collection's modules import, which the base leaves out. That is what makes a play that converges the cluster with `cargoship_apply` and then applies manifests to it one play rather than two images. Point the collection at the kubeconfig cargoship fetched -- `K8S_AUTH_KUBECONFIG` in the environment, or the `kubeconfig` parameter on the task -- and mount that path writable, as below.
