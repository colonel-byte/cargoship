...
Encrypted credentials are the exception. Both age and Ansible Vault salt each encryption, so encrypting one password twice produces two different blocks, and two entries carrying the same credential would read as a mismatch for as long as they are still ciphertext -- which is the whole time this check runs, since it runs before anything is decrypted. So when every credential field both entries set is encrypted, and neither sets `token`, cargoship takes them as agreeing rather than reporting a conflict that exists only in the ciphertext. Everything else is compared as written: plaintext credentials that differ still conflict, differing TLS settings still conflict, and so does a `token` on either side, encrypted or not. A token is a whole credential by itself, so two of them cannot be assumed to be one credential encrypted twice.

Writing `pass: hunter2` in plaintext works, but puts a real credential in the inventory file. Cargoship also accepts an encrypted value in `user`, `pass`, `token`, or `tls.ca`, in either of two formats: an Ansible Vault block starting with `$ANSIBLE_VAULT`, or an age block starting with `-----BEGIN AGE ENCRYPTED FILE-----`. The header tells them apart, so nothing has to be told which format a file uses, and a file part-way through a migration between them applies the same as one that is not. Either is decrypted automatically when the package is applied. The rest of this page uses Ansible Vault; see the [age guide](age-encryption.md) for the age format and its keys.
