...
- **`spec.config.files` and `spec.config.os.files`** -- per file, when the entry sets `template: true`. It defaults to false, because most files are binaries or archives that a
  template pass would corrupt.
...
`spec.config.os.sysctl`, `spec.config.os.environment`, and `spec.config.os.fapolicyd` do not
render.

A template that names a value the package does not define is an error, not an empty string, on
every surface.
