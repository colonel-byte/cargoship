...
The names are the chart names the engine itself uses, and which ones a build packages is a property of that build rather than of the engine: RKE2 1.37 carries `rke2-gateway-api-crd` and `rke2-security-responder`, RKE2 1.34 carries neither, and K3s calls its charts by bare names (`coredns`, `servicelb`, `traefik`, `local-storage`, `metrics-server`, `runtimes`). Rather than keep a list here that drifts from what is packaged, every example's `values.schema.json` carries its own version's charts as suggestions under `addons.disabled`, which an editor offers as completions. That list is generated from the engine's source -- see [mage](../dev/mage.md) and [thirdparty-src](../dev/thirdparty-src.md) -- so it is the vocabulary that build advertises. Read the schema next to the `distro.yaml` you are installing.

**The suggestions are not the whole vocabulary.** The engine's `--disable` help text is where the list comes from, and it is not exhaustive: RKE2 bundles `rke2-runtimeclasses` and never names it there. A schema that rejected everything outside the list would reject a chart the engine would have honored, so `addons.disabled` accepts any chart name and the list only drives completion. An unknown name reaches `config.yaml` unchanged: cargoship warns that the build packages no such component and leaves the entry in place, since dropping it would quietly install the very chart the cluster asked to be without. A typo is visible in that warning rather than at validation time.
