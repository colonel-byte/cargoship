...
Merge keys work the same way. A registry that merges a shared block reads whatever that block holds, encrypted or not, and the keys it writes beside the merge are its own values, encrypted separately:
