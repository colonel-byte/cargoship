...
`.github/workflows/fix-vendor-osv.yaml` makes that commit instead. It is the one workflow here that runs on `pull_request_target`, because it needs a token that can push to the pull request branch and Dependabot's `pull_request` runs are handed a read-only one. That trigger is also why the job never fetches the pull request at all: it checks out the base branch, runs `Dev.WriteOSVOverrides` against that tree, and writes the four results onto the pull request branch through the git data API -- a blob per changed override, a tree on top of the branch's own tree, a commit, and a fast-forward update of the ref. Putting a freshly bumped dependency's tree on a runner that holds a write token is the specific failure `pull_request_target` is known for, and the API route means it never lands there to begin with. Generating from base is sound because the file contents come from `osvOverrides` in `magefiles/vendor-osv.go`, and a dependency bump never touches that file.

An earlier version checked the pull request out beside the base branch and pushed with `git`. It never ran anything out of that checkout either, but Scorecard's Dangerous-Workflow check cannot see that: its `validateUntrustedCodeCheckout` flags any `actions/checkout` in a `pull_request_target` workflow whose `ref` mentions `github.event.pull_request`, which caught both the head checkout and the base one that named `base.sha` -- a trusted commit. The check scored 0 and dropped the overall score by about a point. Dropping `ref` from the base checkout is not a workaround: on `pull_request_target` the default checkout is already the base branch tip, so naming it bought nothing and claimed something the job does not do.

The commit is built with explicit author and committer fields rather than `git commit`, so the `[bot]` identity and the `Signed-off-by` trailer the DCO check wants are carried the same way they were. The ref update is a plain PATCH with no force: the parent is whatever `git/ref/heads/<branch>` returned moments earlier, so a Dependabot push that lands mid-run makes the update fail instead of overwriting the newer commit.
