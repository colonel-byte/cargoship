...
So the phase walk *is* the install. `Test_00_CreatePackage` and `Test_01_Prepare` still shell out to the binary, the phase tests then perform the install in-process, and the CLI `apply` survives only as `Test_ZZ2_ApplyIsIdempotent`, which runs after the cluster is already up. That invocation now earns its place twice over: it is the idempotency check *and* the only test that exercises `action.NewApply`'s own wiring -- the phase list, the flag plumbing, the ordering -- which the phase tests deliberately bypass.
...
**There are two generated inventory files.** `generated-cluster-full.yaml` has all ten machines and is what the harness is built from; `generated-cluster.yaml` has the nine that join and is what `e2e.ClusterConfigPath` points at. The CLI-driven steps -- `Test_ZZ2_ApplyIsIdempotent` and `Test_ZZ3_Reset` -- shell out to the binary with that path, and the binary has no notion of an upload-only host, so it would try to install the engine on Alpine. A single file with an annotation the binary ignores was considered and rejected: it would put a test-only concept into the inventory schema.
