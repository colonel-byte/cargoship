...
*   **`example-release-lines.go`:** The cache behind `fetchReleaseLines`: where it lives, how an entry is keyed and trusted, and the atomic write that keeps a half-written entry from being read back as a whole asset.
