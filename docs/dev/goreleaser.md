...
*   **The rpm's own signature is verified at build time.** `crypto/software@conlon.dev.gpg` reaches the context through the entry's `extra_files`, is `COPY`'d to `/etc/pki/rpm-gpg/RPM-GPG-KEY-colonel-byte`, and `rpm --import` puts it in the image keyring so `rpm --checksig` passes ahead of `rpm --install`. Without the import the install still succeeds but logs `NOKEY`, which is a warning rather than a failure — an unsigned or wrongly signed package would reach an operator unremarked. The `--checksig` step is what makes that case fail the release instead.
...
*   **The apk's own signature is verified at build time.** `crypto/software@conlon.dev.rsa.pub` reaches the context through the entry's `extra_files` — see [The Dockerfile Contract](#the-dockerfile-contract) — and is `COPY`'d to `/etc/apk/keys/software@conlon.dev.rsa.pub`. That path is not a choice: nfpm records the signature as the tar member `.SIGN.RSA.software@conlon.dev.rsa.pub` and apk resolves that name under `/etc/apk/keys`, so the filename in `crypto/` is already the filename apk wants and the `COPY` is a plain copy. `apk verify` then runs ahead of the install, mirroring `rpm --checksig` in the UBI image. It is belt-and-braces — `apk add` refuses an untrusted package on its own — but it fails naming the signature rather than the install, and it proves the key matches rather than leaving that inferred. **`--allow-untrusted` must not come back**: with the key present it is not needed, and it would silently turn both checks off.
...
*   **No Kubernetes tooling.** `kubernetes.core` is in the base, because `alpine/ansible` carries the full ansible distribution rather than ansible-core alone, but the Python library its modules import is not, and the image does not add it — nor `helm` or `kubectl`. That tooling ran to roughly 200 MB on a 483 MB base and served plays the image does not have to run: it installs a cluster, it does not administer one. An operator who wants it builds `FROM` this image.
