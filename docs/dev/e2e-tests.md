...
test/e2e/cluster/      the install group: a nine-node bootloose cluster (plus one upload-only node), walked one phase at a time -- install, join, optionally upgrade, then reset
