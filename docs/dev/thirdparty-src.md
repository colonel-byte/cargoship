...
`src/pkg/engineconfig/extract/components.go` does the parsing: `StringListDecl` handles both the const-string and `[]string{...}` shapes, and `DisableSetCalls` scans for the `Set("disable", ...)` calls.
