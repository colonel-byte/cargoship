...
## cargoship registry-sync

Sync registry config to a cluster, draining and restarting the engine service on any node whose config has drifted

```
cargoship registry-sync [flags]
```
...
```
  ...
  -D, --distro string                What type of distro that will have its registry config synced. Valid options are: 'rke2', 'k3s'.
  -h, --help                         help for registry-sync
